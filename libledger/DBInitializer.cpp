...
dev::storage::Storage::Ptr DBInitializer::initDMStorage()
{
    DBInitializer_LOG(INFO) << LOG_BADGE("initStorageDB") << LOG_BADGE("initDMStorage");

    // exit when enable the unsupported features
    unsupportedFeatures("DMStorage");

    auto dmStorage = createDMStorage(m_param, [](std::exception& e) {
        DBInitializer_LOG(ERROR) << LOG_BADGE("STORAGE") << LOG_BADGE("DM")
...
dev::storage::Storage::Ptr dev::ledger::createDMStorage(
    std::shared_ptr<LedgerParamInterface> _param,
    std::function<void(std::exception& e)> _fatalHandler)
{
    DMConnectionPoolConfig connectionConfig{_param->mutableStorageParam().dbType,
        _param->mutableStorageParam().dbIP, _param->mutableStorageParam().dbPort,
        _param->mutableStorageParam().dbUsername, _param->mutableStorageParam().dbPasswd,
        _param->mutableStorageParam().dbName, _param->mutableStorageParam().dbCharset,
        _param->mutableStorageParam().initConnections,
        _param->mutableStorageParam().maxConnections};
    auto dmStorage = std::make_shared<DMStorage>();

    auto sqlconnpool = std::make_shared<DMConnectionPool>();
    sqlconnpool->createDataBase(connectionConfig);
    sqlconnpool->InitConnectionPool(connectionConfig);

    auto sqlAccess = std::make_shared<DMBasicAccess>();
    dmStorage->SetSqlAccess(sqlAccess);
    dmStorage->setConnPool(sqlconnpool);

    dmStorage->setFatalHandler(_fatalHandler);
