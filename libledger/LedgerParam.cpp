...
void LedgerParam::initStorageConfig(ptree const& pt)
{
    ...
    if (!dev::stringCmpIgnoreCase(mutableStorageParam().type, "mysql")){
    ...
    if (!dev::stringCmpIgnoreCase(mutableStorageParam().type, "gbase")){
        mutableStorageParam().dbType = pt.get<std::string>("storage.db_type", "gbase");
    }
