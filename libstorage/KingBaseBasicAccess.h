...
// const static uint32_t maxKingBasePlaceHolderCnt = 60000;
const static uint32_t maxKingBasePlaceHolderCnt = 90;
#define KingBaseBasicAccess_LOG(LEVEL) LOG(LEVEL) << "[KingBaseBasicAccess] "
...
struct KingBasePlaceholderItem
{
    std::string sql;
    uint32_t placeholderCnt;

    KingBasePlaceholderItem() : placeholderCnt(0) {}
};

class KingBaseBasicAccess
{
public:
    KingBaseBasicAccess();
    virtual ~KingBaseBasicAccess() {}
    typedef std::shared_ptr<KingBaseBasicAccess> Ptr;
    ...
    virtual bool JudgeIfCreateUnique();
...
private:
    ...
    std::vector<KingBasePlaceholderItem> BuildCommitSql(const std::string& _table,
    ...
    KingBaseFieldType getFieldType(std::string const& _tableName);
    bool inline isBlobType(std::string const& _tableName)
    {
        auto fieldType = getFieldType(_tableName);
        if (fieldType == KingBaseFieldType::KMediumBlobType || fieldType == KingBaseFieldType::KLongBlobType)
        {
            return true;
        }
        return false;
...
public:
    ...
    void setConnPool(std::shared_ptr<KingBaseConnectionPool>& _connPool);

private:
    std::shared_ptr<KingBaseConnectionPool> m_connPool;
