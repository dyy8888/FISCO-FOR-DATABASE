...
bool DMConnectionPool::InitConnectionPool(const storage::DMConnectionPoolConfig& _dbConfig)
{
    if (_dbConfig.dbType == "dm")
    {   //用于进行字符串的输入和输出操作
        stringstream ss;
...
*/
Connection_T DMConnectionPool::GetConnection()
{
...
*/
int DMConnectionPool::ReturnConnection(const Connection_T& _connection)
{
    ConnectionPool_returnConnection(m_connectionPool, _connection);
    return 0;
}

int DMConnectionPool::BeginTransaction(const Connection_T& _connection)
{
    Connection_beginTransaction(_connection);
    return 0;
}

int DMConnectionPool::Commit(const Connection_T& _connection)
{
    Connection_commit(_connection);
    return 0;
}

int DMConnectionPool::RollBack(const Connection_T& _connection)
{
...
{
    DMConnectionPool_LOG(ERROR) << _exitInfo.str();
    raise(SIGTERM);
    BOOST_THROW_EXCEPTION(StorageException(-1, _exitInfo.str()));
}
//析构函数
DMConnectionPool::~DMConnectionPool()
{
    if (m_connectionPool)
    {
...
int DMConnectionPool::GetActiveConnections()
{
    return ConnectionPool_active(m_connectionPool);
}

int DMConnectionPool::GetMaxConnections()
{
    return ConnectionPool_getMaxConnections(m_connectionPool);
}

int DMConnectionPool::GetTotalConnections()
{
    return ConnectionPool_size(m_connectionPool);
}

void DMConnectionPool::createDataBase(const DMConnectionPoolConfig& _dbConfig)
{
    if (_dbConfig.dbType == "dm")
    {
        stringstream ss;
