 ...
 */
/** @file KingBaseConnectionPool.cpp
 *  @author darrenyin
 *  @date 2019-04-25
 */
...
bool KingBaseConnectionPool::InitConnectionPool(const storage::KingBaseConnectionPoolConfig& _dbConfig)
{
    if (_dbConfig.dbType == "kingbase")
    {   //用于进行字符串的输入和输出操作
        stringstream ss;
        ...
        KingBaseConnectionPool_LOG(DEBUG)
        ...
        CATCH(SQLException)
        {
            KingBaseConnectionPool_LOG(ERROR)
...
*/
Connection_T KingBaseConnectionPool::GetConnection()
{
...
*/
int KingBaseConnectionPool::ReturnConnection(const Connection_T& _connection)
{
    ConnectionPool_returnConnection(m_connectionPool, _connection);
    return 0;
}

int KingBaseConnectionPool::BeginTransaction(const Connection_T& _connection)
{
    Connection_beginTransaction(_connection);
    return 0;
}

int KingBaseConnectionPool::Commit(const Connection_T& _connection)
{
    Connection_commit(_connection);
    return 0;
}

int KingBaseConnectionPool::RollBack(const Connection_T& _connection)
{
...
{
    KingBaseConnectionPool_LOG(ERROR) << _exitInfo.str();
    raise(SIGTERM);
    BOOST_THROW_EXCEPTION(StorageException(-1, _exitInfo.str()));
}
//析构函数
KingBaseConnectionPool::~KingBaseConnectionPool()
{
    if (m_connectionPool)
    {
...
int KingBaseConnectionPool::GetActiveConnections()
{
    return ConnectionPool_active(m_connectionPool);
}

int KingBaseConnectionPool::GetMaxConnections()
{
    return ConnectionPool_getMaxConnections(m_connectionPool);
}

int KingBaseConnectionPool::GetTotalConnections()
{
    return ConnectionPool_size(m_connectionPool);
}

void KingBaseConnectionPool::createDataBase(const KingBaseConnectionPoolConfig& _dbConfig)
{
    if (_dbConfig.dbType == "kingbase")
    {
        stringstream ss;
