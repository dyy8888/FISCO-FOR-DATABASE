...
#define GBaseStorage_LOG(LEVEL) LOG(LEVEL) << "[GBaseStorage] " //宏指令，用于记录日志

namespace dev
{
namespace storage
{
class GBaseStorage : public Storage
{
public:
    typedef std::shared_ptr<GBaseStorage> Ptr;    //构造一个共享指针，指向ZabStorage

    GBaseStorage();
    virtual ~GBaseStorage(){};
